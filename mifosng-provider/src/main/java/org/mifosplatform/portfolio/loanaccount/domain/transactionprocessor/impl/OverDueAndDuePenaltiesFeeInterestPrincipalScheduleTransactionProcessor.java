...
/**
 * OverDueAndDuePenaltiesFeeInterestPrincipal style {@link LoanRepaymentScheduleTransactionProcessor}.
 * 
 * Per OverDueAndDuePenaltiesFeeInterestPrincipal regulations, all interest must be paid (both current and overdue as on monthly bases)
 ...
 */
public class OverDueAndDuePenaltiesFeeInterestPrincipalScheduleTransactionProcessor extends AbstractLoanRepaymentScheduleTransactionProcessor {
    ...
    protected Money handleTransactionThatIsALateRepaymentOfInstallment(final LoanRepaymentScheduleInstallment currentInstallment,
        ...
        if (loanTransaction.isInterestWaiver()) {
           ...
           // int loanTerm=loanTransaction.getLoan().getTermFrequency();//new code By Venkat
            for (final LoanRepaymentScheduleInstallment installment : installments) {
                if (installment.isInterestDue(currency)
                        && (installment.isTxnDateInCurrentInstallment(loanTransaction.getTransactionDate()) || installment.getInstallmentNumber().equals(
...
/*                    //Newly added Venkat - from here
                    if (installment.getInstallmentNumber() >= installments.size() &&
                            !(transactionDate.isBefore(installment.getDueDate()))) {
                        LocalDate dateForInterestCalculation = transactionDate.dayOfMonth().withMaximumValue();
                        interestPortion = installment.payInterestComponent(dateForInterestCalculation, transactionAmountRemaining);
                    } else {                                  
                        interestPortion = installment.payInterestComponent(transactionDate, transactionAmountRemaining);
                    }
                  //Newly added Venkat - till here
*/               // Original: 
